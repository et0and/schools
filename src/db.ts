...
export class DatabaseService {
  ...
  async querySchools(query: SchoolQuery): Promise<{ schools: School[]; total: number }> {
    const { page, limit, name, city, suburb, authority, status, org_type } = query
  ...
  async searchSchoolsByName(searchTerm: string, limit: number, offset: number): Promise<{ schools: School[]; total: number }> {
    const totalResult = await this.db
      .select({ count: sql<number>`count(*)` })
      .from(schools)
