...
export class RateLimitService {
  ...
  async checkRateLimit(identifier: string): Promise<{ allowed: boolean; remaining: number; resetTime: number }> {
    ...
    try {
      ...
      const remaining = Math.max(0, this.config.maxRequests - requestData.count - 1)
  ...
  async getRateLimitInfo(identifier: string): Promise<{ remaining: number; resetTime: number; total: number }> {
    const key = this.getKey(identifier)
    const now = Date.now()
